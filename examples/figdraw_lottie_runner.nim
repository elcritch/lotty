...
when isMainModule:
  when not defined(emscripten):
    if paramCount() < 1:
      echo "Usage: figdraw_lottie_runner <lottie.json>"
      quit(1)

  var app_running = true

  let title = "figdraw: Lottie Runner"
  ...
  when defined(emscripten):
    let lottiePath = figDataDir() / "bouncy_ball.json"
  else:
    let lottiePath = paramStr(1)

  let anim = loadLottieFile(lottiePath)
